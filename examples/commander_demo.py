...
import sys, time, signal, os, yaml
...
class GeneralCommanderDemo():
    ...
    def _print_objects(self):
        ...
        for object in objects:
            logger.info(f'Object: {self.arm_commander.scene.get_object_poses([object])}')
    ...
    def test_1(self):
        try:
            ...
            logger.info(f'=== Test 1: move commands and collision detection')
            ...
            # Move a displacement
            logger.info(f'move_displacement: dx')
            arm_commander.move_displacement(dx = 0.2, wait=True)
            arm_commander.reset_state() 
            logger.info(f'move_displacement: dy')       
            arm_commander.move_displacement(dy = 0.2, wait=True)
            arm_commander.reset_state()   
            logger.info(f'move_displacement: dz')    
            arm_commander.move_displacement(dz = 0.1, wait=True)
            arm_commander.reset_state()         

            # Move to a position using path planner
            logger.info(f'move_to_position: path mode, world frame x = 0.4')
            arm_commander.move_to_position(x = 0.4, wait=True)
            arm_commander.reset_state()           
            logger.info(f'move_to_position: path mode, world frame x = 0.2 y = -0.3')            
            arm_commander.move_to_position(x = 0.2, y = -0.3, wait=True)
            arm_commander.reset_state()       
            logger.info(f'move_to_position: path mode, world frame z = 0.8')
            ...
            arm_commander.move_to_named_pose('home', wait=True)
            arm_commander.reset_state()
            logger.info(f'move_to_position: cartesian mode, world frame x = 0.5')
            arm_commander.move_to_position(x = 0.5, cartesian=True, wait=True)
            arm_commander.reset_state()           
            logger.info(f'move_to_position: cartesian mode, world frame x = 0.3, y = 0.3')            
            arm_commander.move_to_position(x = 0.3, y = 0.3, cartesian=True, wait=True, accept_fraction=0.6)
            arm_commander.reset_state()       
            logger.info(f'move_to_position: cartesian mode, world frame x = 0.3, y = -0.4, z = 0.6')
            arm_commander.move_to_position(x = 0.3, y = -0.4, z = 0.6, cartesian=True, wait=True, accept_fraction=0.9)
            arm_commander.reset_state()
            logger.info(f'move_to_position: cartesian mode, world frame x = 0.6, y = 0.2, z = 0.3')
            ...
            arm_commander.move_to_named_pose('home', wait=True)
            arm_commander.reset_state()
            logger.info(f'current pose: {arm_commander.pose_in_frame_as_xyzrpy(reference_frame="the_box")[:3]}')
            logger.info(f'move_to_position: path mode, box frame (0, 0, 0.35)')
            arm_commander.move_to_position(x = 0, y = 0, z = 0.35, reference_frame='the_box', wait=True)
            arm_commander.reset_state()       
            logger.info(f'current pose: {arm_commander.pose_in_frame_as_xyzrpy(reference_frame="the_box")[:3]}')            
            logger.info(f'move_to_position: path mode, box frame z = 0.65')            
            arm_commander.move_to_position(z = 0.65, reference_frame='the_box', wait=True)
            arm_commander.reset_state()       
            logger.info(f'current pose: {arm_commander.pose_in_frame_as_xyzrpy(reference_frame="the_box")[:3]}')
            logger.info(f'move_to_position: path mode, box frame x = 0.25, z = 0.31')
            ...
            logger.info(f'current orientation (the box): {arm_commander.pose_in_frame_as_xyzrpy(reference_frame="the_box")[3:]}')
            logger.info(f'current orientation: {arm_commander.pose_in_frame_as_xyzrpy()[3:]}') 
            logger.info(f'rotate (the_box): yaw = 3.14')
            arm_commander.rotate_to_orientation(yaw=-3.14, reference_frame='the_box')
            arm_commander.reset_state()  
            
            self.arm_commander.clear_path_constraints()   
            
            logger.info(f'rotate (the_box): roll = -2.8')
            arm_commander.rotate_to_orientation(roll=-2.8, reference_frame='the_box')  
            arm_commander.reset_state() 
            logger.info(f'rotate (world): yaw = -3.4')
            arm_commander.rotate_to_orientation(yaw=-3.14)  
            arm_commander.reset_state()  
            logger.info(f'rotate (the_box): roll = -3.14, yaw = 3.14')
            ...
            arm_commander.move_to_named_pose('home', wait=True)
            arm_commander.reset_state()
            logger.info(f'collision with self (will be aborted)')
            arm_commander.move_to_position(x = 0.0, y = 0.0, z = 0.0, wait=True)
            arm_commander.reset_state()

            # Collision with the wall
            logger.info(f'collision with the wall (will be aborted)')
            ...
            logger.info(f'collision with the box (will be aborted)')
            ...
            logger.info(f'collision avoidance of a sphere')
            the_sphere = self.demo_config['scene']['the_sphere']
            arm_commander.add_sphere_to_scene('the_sphere', the_sphere['dimensions'], 
                                              the_sphere['position'])
            time.sleep(1.0)
            
            logger.info(f'move to the other side of the sphere')
            arm_commander.move_to_position(y = the_sphere['position'][1] + 0.15, wait=True)
            arm_commander.reset_state()
            
            logger.info(f'return to home pose')
            arm_commander.move_to_named_pose('home', wait=True)
            arm_commander.reset_state()    
            
            logger.info(f'move to the other side of the sphere by cartesian (will be aborted)')
            arm_commander.move_to_position(y = the_sphere['position'][1] + 0.15, cartesian=True, wait=True)
            arm_commander.reset_state() 
            
            logger.info(f'move to the other side of the sphere with orientation constraint')
            arm_commander.add_path_constraints(moveit_tools.create_path_orientation_constraint(
    ...
    def test_2(self):
        try:
            ...
            logger.info(f'=== Test 2: asynchronous commands')
            
            arm_commander.reset_state()
            arm_commander.move_to_named_pose('stow', wait=False)
            logger.info('move to stow command has been sent to the commander')
            while True:
   ...
   # test position constraint
    def test_3(self):
        try:
            arm_commander = self.arm_commander
            logger.info(f'=== Test 3: position constraints')
            # set workspace and named_poses
            arm_commander.reset_world()
            arm_commander.reset_state()
            arm_commander.set_workspace_walls(*(self.demo_config['scene']['workspace']))
            ...
            logger.info(f'current pose of the end-effector in xyzrpy: {arm_commander.pose_in_frame_as_xyzrpy()}')
            ...
            logger.info(f'return to home') 
            arm_commander.move_to_named_pose('home', wait=True)
            arm_commander.reset_state()

            current_pose: PoseStamped = arm_commander.pose_in_frame(end_effector_link)
            current_pose_xyzrpy: list = arm_commander.pose_in_frame_as_xyzrpy(end_effector_link)
            logger.info(f'Current pose of the end-effector in xyzrpy: {current_pose_xyzrpy}')
            ...
            the_constraint = moveit_tools.create_position_constraint_on_link_orientation(arm_commander.END_EFFECTOR_LINK, current_pose, 
                                            [1.0, 0.1, 0.1])
            logger.info(f'added position constraint defined in link orientation')
            arm_commander.add_path_constraints(the_constraint)

            # violate the position constraint
            logger.info(f'move to y + 0.1 (violated)') 
            arm_commander.move_to_position(y=current_pose_xyzrpy[1] + 0.1, wait=True)   
            arm_commander.reset_state()

            logger.info(f'move to y - 0.2 (violated)') 
            arm_commander.move_to_position(y=current_pose_xyzrpy[1] - 0.2, wait=True)   
            arm_commander.reset_state()
            
            # agree with the position constraint
            logger.info(f'move to x + 0.2 and y - 0.2') 
            ...
            logger.info(f'added position constraint defined as bbox [0.2, -0.5, 0.2, 0.8, 0.5, 0.6]')
            arm_commander.add_path_constraints(the_constraint)
            
            # violate the position constraint
            logger.info(f'move to x = 0.1 (violated)') 
            arm_commander.move_to_position(x = 0.1, wait=True)   
            arm_commander.reset_state()           
            
            logger.info(f'move to y = 0.6 (violated)') 
            arm_commander.move_to_position(y = 0.6, wait=True)   
            arm_commander.reset_state()    
            
            logger.info(f'move to z = 0.7 (violated)') 
            arm_commander.move_to_position(z = 0.7, wait=True)   
            arm_commander.reset_state()          
            
            # agree with the position constraint      
            logger.info(f'move to x = 0.65') 
            arm_commander.move_to_position(x = 0.65, wait=True)   
            arm_commander.reset_state()           
            
            logger.info(f'move to y = -0.3') 
            arm_commander.move_to_position(y = -0.3, wait=True)   
            arm_commander.reset_state()    
            
            logger.info(f'move to z = 0.5') 
        ...
        try:
            ...
            if arm_commander.controller_select(controller_type=ControllerState.SERVO):
                ...
                if arm_commander.controller_select(controller_type = ControllerState.TRAJECTORY):
                    logger.info(f"Successfully Switched back to TRAJECTORY Controller")
                else:
                    logger.error(f"Failed to Switch back to TRAJECTORY Controller")
            else:
                logger.warning(f"Failed to Switch to SERVO Controller")

            arm_commander.move_to_named_pose('home', wait=True)
            arm_commander.reset_state() 
            logger.info(f'=== Test 4: Finished')
